...
class Breadcrumbs {
	...
	function push($page, $href, $index)
	{
		...
		// push breadcrumb
		$this->breadcrumbs[$href] = array('page' => $page, 'href' => $href, 'index' => $index);
	}
	 ...
	 * @return	void
	 */		
	function unshift($page, $href, $index)
	{
		// no crumb provided
		if (!$page OR !$href) return;
		...
		// add at firts
		array_unshift($this->breadcrumbs, array('page' => $page, 'href' => $href, 'index' => $index));
	}
		...
		if ($this->breadcrumbs) {
			...
			foreach ($this->breadcrumbs as $key => $crumb) {
				...
				if (end($keys) == $key)
				{
					...
					//$output .= $this->crumb_open.'<a href="' . $crumb['href'] . '" style="z-index:'.$crumb['index'].'">' . $crumb['page'] . '</a> '.$this->crumb_divider.$this->crumb_close;
					$output .= $this->crumb_open.'<a href="' . $crumb['href'] . '" style="z-index:'.$crumb['index'].'">' . $crumb['page'] . '</a> '.$this->crumb_close;
				}
			}
